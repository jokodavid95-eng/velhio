    ...
    <>
      <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:200,padding:'1.6rem 4rem',display:'flex',justifyContent:'space-between',alignItems:'center',background:'transparent',pointerEvents:'none'}}>
        ...
        <ul style={{display:'flex',gap:'0.5rem',listStyle:'none',margin:0,padding:0,pointerEvents:'auto'}}>
          {[['#historia','Historia'],['#favoritas','Preferidas'],['#carta','Carta'],['#contacto','Contacto']].map(([href,label])=>(
            <li key={href}><a href={href} style={{fontSize:'0.68rem',letterSpacing:'0.15em',textTransform:'uppercase',color:'rgba(245,239,224,0.9)',textDecoration:'none',fontFamily:"'Jost',sans-serif",background:'rgba(255,255,255,0.08)',backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)',border:'1px solid rgba(212,168,67,0.2)',borderRadius:'2rem',padding:'0.4rem 1rem',display:'block'}}
              ...
              onMouseLeave={e=>{e.currentTarget.style.background='rgba(255,255,255,0.08)';e.currentTarget.style.borderColor='rgba(212,168,67,0.2)'}}
            >{label}</a></li>
          ))}
        </ul>
      ...
      <div className="mobile-bottom-bar" style={{position:'fixed',bottom:'1.2rem',left:'50%',transform:'translateX(-50%)',zIndex:9999,display:'flex',justifyContent:'center',alignItems:'center',gap:'0.5rem'}}>
        {[
          ...
          <a key={item.href} href={item.href} target={item.ext?'_blank':undefined} rel={item.ext?'noopener noreferrer':undefined} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'0.2rem',textDecoration:'none',background:item.ext?'#D4A843':'rgba(255,255,255,0.1)',backdropFilter:'blur(16px)',WebkitBackdropFilter:'blur(16px)',border:item.ext?'1px solid rgba(212,168,67,0.6)':'1px solid rgba(212,168,67,0.25)',borderRadius:'2rem',padding:'0.5rem 0.8rem 0.45rem',minWidth:'3.2rem',boxShadow:'0 4px 20px rgba(0,0,0,0.3)'}}>
